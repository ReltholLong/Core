...
void KNpc::ServeMove(int MoveSpeed)
{
	...
	// Detect toggle pattern cho player movement
	if (IsPlayer())
	{
		static struct {
			int coord1[2], coord2[2];
			int lastPos; // 1 ho?c 2
			int toggles;
			DWORD startTime;
		...
		// Ch? check khi di chuy?n m?t kho?ng ng?n (bot pattern)
		int currentX = m_MapX;
		int currentY = m_MapY;

		if (pattern.lastPos == 0)
		{
			pattern.coord1[0] = currentX;
			pattern.coord1[1] = currentY;
			pattern.lastPos = 1;
			pattern.startTime = currentTime;
		}
		else if (pattern.lastPos == 1)
		{
			if (currentX != pattern.coord1[0] || currentY != pattern.coord1[1])
			{
				pattern.coord2[0] = currentX;
				pattern.coord2[1] = currentY;
				pattern.lastPos = 2;
				pattern.toggles++;
			}
		}
		else if (pattern.lastPos == 2)
		{
			if (currentX == pattern.coord1[0] && currentY == pattern.coord1[1])
			{
				pattern.lastPos = 1;
			...
			else if (currentX != pattern.coord2[0] || currentY != pattern.coord2[1])
			{
				// Ði?m m?i, reset
				pattern.coord1[0] = currentX;
				pattern.coord1[1] = currentY;
		...
		// Log pattern n?u phát hi?n
		if (pattern.toggles > 5 && (currentTime - pattern.startTime < 30000))
		{
			g_DebugLog("[TOGGLE_PATTERN] Player:%s toggles (%d,%d)<->(%d,%d) %d times",
				Name, pattern.coord1[0], pattern.coord1[1],
				pattern.coord2[0], pattern.coord2[1], pattern.toggles);
		}
	}

	// Rest of original ServeMove code...
	int x, y;
	...
	// Rest of movement code gi? nguyên...
	int nOldRegion = m_RegionIndex;
	int nOldMapX = m_MapX;
	int nOldMapY = m_MapY;
	int nOldOffX = m_OffX;
...
int KNpc::SetPos(int nX, int nY)
{
	...
	if (m_Kind == kind_player)
	{
		...
		// CH? LOG KHI CÓ MOVEMENT
		if (moveTracker.lastX != coordX || moveTracker.lastY != coordY)
		{
			g_DebugLog("[SETPOS] Player:%s Move (%d,%d)->(%d,%d)",
				Name, moveTracker.lastX, moveTracker.lastY, coordX, coordY);

			// Ki?m tra suspicious coordinates t? log
			if ((coordX == 1652 && coordY == 3237) || (coordX == 1654 && coordY == 3242))
			{
				g_DebugLog("[SUSPICIOUS_COORD] Player:%s at BOT coordinate (%d,%d)", Name, coordX, coordY);
			}

			// Ki?m tra SetPos spam
			if (currentTime - moveTracker.lastTime < 200) // < 0.2 giây
			{
				moveTracker.moveCount++;
				if (moveTracker.moveCount > 3)
				{
					g_DebugLog("[SETPOS_SPAM] Player:%s rapid SetPos count:%d", Name, moveTracker.moveCount);
				}
			}
			else
			{
				moveTracker.moveCount = 0;
			}

			moveTracker.lastX = coordX;
			moveTracker.lastY = coordY;
			moveTracker.lastTime = currentTime;
		}
	}
...
void KNpc::CheckTrap()
{
	...
	DWORD dwCurrentTrap = SubWorld[m_SubWorldIndex].m_Region[m_RegionIndex].GetTrap(m_MapX, m_MapY);

	// CH? GHI LOG KHI TRAP THAY Ð?I (transition)
	if (m_TrapScriptID != dwCurrentTrap)
	{
		// Log trap transition
		g_DebugLog("[TRAP_TRANSITION] Player:%s From_Trap:%d To_Trap:%d Pos:(%d,%d)",
			Name, m_TrapScriptID, dwCurrentTrap, m_MapX, m_MapY);

		// Ki?m tra combat zone transition
		if ((m_TrapScriptID == 0 && dwCurrentTrap != 0) || (m_TrapScriptID != 0 && dwCurrentTrap == 0))
		{
			g_DebugLog("[COMBAT_ZONE] Player:%s %s at (%d,%d)",
				Name, (dwCurrentTrap != 0 ? "ENTER_COMBAT_ZONE" : "EXIT_COMBAT_ZONE"), m_MapX, m_MapY);
		}

		// Ki?m tra trap spam
		static DWORD lastTrapTime = 0;
		static int trapCount = 0;
		DWORD currentTime = GetCurrentTime();

		if (currentTime - lastTrapTime < 1000) // < 1 giây
		{
			trapCount++;
			if (trapCount > 5)
			{
				g_DebugLog("[TRAP_SPAM] Player:%s rapid trap changes: %d times", Name, trapCount);
			}
		}
		else
		{
