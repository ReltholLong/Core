...
{
	g_DebugLog("[ObjSet]Tai region obj tu %s region (%d,%d)", lpszMapPath, nRegionX, nRegionY);
...
//	g_SetFilePath(szPath);
	...
	for (int i = 0; i < sHead.uNumObj; i++)
	{
		...
		g_DebugLog("[ObjSet]Tao object %d loai %d script: %s",
			nNo, nKind, sData.szScript);
	}
	cDataFile.Close();
...
int	KObjSet::SearchObjAt(int nX, int nY, int nRange)
{
	...
	static int objectsChecked = 0;  // ← Add
	searchCallCount++;
	objectsChecked = 0;             // ← Add

	if (searchCallCount <= 5)  // Log 5 lần đầu
	{
		printf("[SearchObj]Lan %d: tim tai (%d,%d) ban kinh %d\n",
			searchCallCount, nX, nY, nRange);
		g_DebugLog("[SearchObj]Lan %d: tim tai (%d,%d) ban kinh %d",
			searchCallCount, nX, nY, nRange);
	}
	...
	while (1)
	{
		...
		if (Object[nIdx].m_nRegionIdx < 0) continue;

		objectsChecked++;

		// KIỂM TRA: có check Trap không?
			...
			// && Obj_Kind_Trap != Object[nIdx].m_nKind)  // ← THÊM DÒNG NÀY!
		{
			...
			{
				printf("[SearchObj]SKIP TRAP %d - can fix search function!\n", nIdx);
				g_DebugLog("[SearchObj]SKIP TRAP %d - can fix search function!", nIdx);
			}
			continue;
		}
		...
		{
			printf("[SearchObj]Check object %d loai %d\n", nIdx, Object[nIdx].m_nKind);
		}
		...
		if (searchCallCount <= 5)
		{
			printf("[SearchObj]Kiem tra %d objects, tim thay: %d\n",
...
#ifndef _SERVER
//-------------------------------------------------------------------------
//	¹¦ÄÜ£ºÅÐ¶ÏÊÇ·ñÈ«²¿ÏÔÊ¾ item ºÍ money ÀàµÄ object µÄÃû×Ö  ·µ»ØÖµ TRUE ÏÔÊ¾£¬FALSE ²»ÏÔÊ¾
//-------------------------------------------------------------------------
